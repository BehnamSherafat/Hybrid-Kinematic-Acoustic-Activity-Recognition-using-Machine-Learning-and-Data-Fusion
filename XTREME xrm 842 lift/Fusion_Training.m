...
PCA_f = 0.9;
cross_fold = 10;
delay = 6.28; % how much audio is prior to vibration
...
[A_ori, fsA] = audioread('ZOOM0019.wav');

%% Audio Denoising: method = 'mcra2'; 

A_ori(A_ori==0)=10^-4;
A_ori = A_ori(:, 1);
audiowrite('ZOOM0019_ori.wav', A_ori, fsA);

% Audio Denoising: method = 'mcra2'; 
% specsub_ns('ZOOM0019_ori.wav', 'mcra2', 'ZOOM0019_den.wav')

% Read Modified Audio
[S_Audio, fsA] = audioread('ZOOM0019_den.wav');
...
% Categorizing Data
Activity_1 = [Data(s3:f3,:);Data(s6:f6,:);Data(s9:f9,:);Data(s11:f11,:)];
Activity_2 = [Data(s1:f1,:);Data(s5:f5,:);Data(s7:f7,:)];
Activity_3 = [Data(s2:f2,:);Data(s4:f4,:);Data(s8:f8,:);Data(s10:f10,:)];
...
tttt = 1;
time5 = tic;
label1 = predict(Model,testing_data_N_1(:,2:end));
time6 = toc(time5)
comparison_vibration = table(testing_data_N_1(:,1),label1);
C_vibration = confusionmat(testing_data_N_1(:,1),label1)
...
time7 = tic;
label2 = predict(Model,testing_data_N_2(:,2:end));
time8 = toc(time7)
comparison_audio = table(testing_data_N_2(:,1),label2);
C_audio = confusionmat(testing_data_N_2(:,1),label2)
