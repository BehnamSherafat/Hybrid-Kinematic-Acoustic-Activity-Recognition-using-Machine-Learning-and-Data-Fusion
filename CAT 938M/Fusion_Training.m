...
PCA_f = 0.9;
cross_fold = 10;
delay = 0.8; % how much audio is prior to vibration
...
[A_ori, fsA] = audioread('ZOOM0083.wav');

%% Audio Denoising: method = 'mcra2'; 

A_ori(A_ori==0)=10^-4;
A_ori = A_ori(:, 1);
audiowrite('ZOOM0083_ori.wav', A_ori, fsA);

% Audio Denoising: method = 'mcra2'; 
% specsub_ns('ZOOM0083_ori.wav', 'mcra2', 'ZOOM0083_den.wav')

% Read Modified Audio
[S_Audio, fsA] = audioread('ZOOM0083_den.wav');
...
s1 = ceil((0)*lps+1);
f1 = ceil((2-delay)*lps);
s2 = ceil((2-delay)*lps+1);
f2 = ceil((11-delay)*lps);
s3 = ceil((11-delay)*lps+1);
f3 = ceil((12-delay)*lps);
s4 = ceil((12-delay)*lps+1);
f4 = ceil((24-delay)*lps);
s5 = ceil((24-delay)*lps+1);
f5 = ceil((27-delay)*lps);
s6 = ceil((27-delay)*lps+1);
f6 = ceil((40-delay)*lps);
s7 = ceil((40-delay)*lps+1);
f7 = ceil((41-delay)*lps);
s8 = ceil((41-delay)*lps+1);
f8 = ceil((51-delay)*lps);
s9 = ceil((51-delay)*lps+1);
f9 = ceil((62-delay)*lps);
s10 = ceil((62-delay)*lps+1);
f10 = ceil((63-delay)*lps);
s11 = ceil((63-delay)*lps+1);
f11 = ceil((77-delay)*lps);
s12 = ceil((77-delay)*lps+1);
f12 = ceil((82-delay)*lps);
s13 = ceil((82-delay)*lps+1);
f13 = ceil((83-delay)*lps);
s14 = ceil((83-delay)*lps+1);
f14 = ceil((89-delay)*lps);
s15 = ceil((89-delay)*lps+1);
f15 = ceil((94-delay)*lps);
s16 = ceil((94-delay)*lps+1);
f16 = ceil((95-delay)*lps);
s17 = ceil((95-delay)*lps+1);
f17 = ceil((111-delay)*lps);
s18 = ceil((111-delay)*lps+1);
f18 = ceil((T-delay)*lps);
...
% Categorizing Data
Activity_1 = [Data(s3:f3,:);Data(s7:f7,:);Data(s10:f10,:);Data(s13:f13,:);Data(s16:f16,:)];
Activity_2 = [Data(s1:f1,:);Data(s5:f5,:);Data(s4:f4,:);Data(s8:f8,:);Data(s11:f11,:);Data(s14:f14,:);Data(s17:f17,:);Data(s18:f18,:)];
Activity_3 = [Data(s2:f2,:);Data(s6:f6,:);Data(s9:f9,:);Data(s12:f12,:);Data(s15:f15,:)];
